...
import { X, Sparkles, CheckCircle2, ArrowRight, Building2, User, Mail, Phone, Globe, Clock, MapPin, AlertCircle, Bot, RefreshCw } from "lucide-react";
import { validateEmail } from "@/utils/emailValidation";

interface RequestWebsiteModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function RequestWebsiteModal({ isOpen, onClose }: RequestWebsiteModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    fetch("/api/submit", {
      ...
      .catch((err) => console.error("Error submitting website request form:", err))
      .finally(() => {
        ...
        {!isSubmitted ? (
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-xl font-bold text-white">
                  Request a Free Custom Website
                </h3>
              </div>
            </div>

            <p className="text-sm text-zinc-400 leading-relaxed mb-6">
              Don't see your business in our directory yet? Submit your Google Maps business link and details below. Our Devinsol design team will build your custom website for free within 3 days and publish it to the directory for you!
            </p>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Apex Health Clinic / Metro Pizza"
                  value={businessName}
              ...
              {/* REQUIRED GOOGLE MAPS BUSINESS LINK */}
              <div>
                <label className="block text-xs font-semibold text-zinc-300 mb-1.5 flex items-center gap-1.5">
                ...
                <input
                  type="url"
                  required
                  placeholder="https://maps.google.com/?cid=... or Google Maps share link"
                  value={googleMapsUrl}
                  onChange={(e) => setGoogleMapsUrl(e.target.value)}
                  className="w-full rounded-xl border border-white/10 bg-zinc-900 px-4 py-2.5 text-sm text-white placeholder-zinc-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
                <p className="mt-1 text-[11px] text-zinc-500">
                  Paste your Google Maps listing URL so our team can pull your business location and hours.
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="Tell us key services, colors, or details you'd like on your website..."
                  value={notes}
              ...
              <div className="flex items-center gap-2 rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 text-xs text-blue-300/90">
                <Clock className="h-4 w-4 text-blue-400 shrink-0" />
                <span><strong>3-Day Turnaround:</strong> Our team will build your custom page for free and notify you when it's added to the directory.</span>
              </div>
          ...
          <div className="text-center py-4">
            ...
            <p className="text-sm text-zinc-300 mb-6 max-w-sm mx-auto leading-relaxed">
              Thank you <strong className="text-white">{contactName}</strong>! Our Devinsol design team is crafting a free website for <strong className="text-blue-400">{businessName}</strong>. We will add it to the directory within 3 days and notify <strong className="text-white">{email}</strong>.
            </p>

            <div className="rounded-xl border border-white/10 bg-zinc-900 p-4 text-left text-xs text-zinc-400 mb-6 space-y-2">
              <div className="flex justify-between">
                <span>Request ID:</span>
                <span className="font-mono text-zinc-200">REQ-DEV-{Math.floor(100000 + Math.random() * 900000)}</span>
              </div>
              <div className="flex justify-between">
